...
template<class VFLTR> void	DOWNSAMPLETB<VFLTR>::sync(void) {
	...
	for(int k=0; !syncd && k < (NTAPS()+1) / m_ndown; k++) {
		TESTB<VFLTR>::m_core->i_ce      = 1;
		tick();
		for(int i=0; !syncd && i<ncks + 15; i++) {
			TESTB<VFLTR>::m_core->i_ce      = 0;
	...
	TESTB<VFLTR>::m_core->i_ce      = 0;
	for(int i=0; i<ncks + 15; i++) {
		assert(!TESTB<VFLTR>::m_core->o_ce);
		tick();
	}
}
...
// printf("DOWNSAMPLETB::apply(%d, ...)\n", nlen);
	...
	for(int i=0; i<inlen; i++) {
		...
		for(int k=0; k<nclks; k++) {
			// Apply the filter
			tick();

			// Sign extend any result
			if (TESTB<VFLTR>::m_core->o_ce)
				data[outln++] = sbits(TESTB<VFLTR>::m_core->o_result, OW());
...
template<class VFLTR> void	DOWNSAMPLETB<VFLTR>::load(int  ntaps, int64_t *data) {
	...
	for(int i=0; i<ntaps; i++) {
		// Strip off any excess bits
		if (REVERSE)
			TESTB<VFLTR>::m_core->i_tap   = ubits(data[ntaps-1-i], TW());
		else
...
template<class VFLTR> void	DOWNSAMPLETB<VFLTR>::test(int  &nlen, int64_t *data) {
	...
	for(int i=0; i<tstcounts; i++) {
		...
		if (debug) {
			printf("%c%3d, %d : Input :%10ld[%08lx] ",
				(i >= inlen)?'z':' ', i, inlen,
				sbits(TESTB<VFLTR>::m_core->i_sample,IW()),
				ubits(TESTB<VFLTR>::m_core->i_sample,IW()) );
			fflush(stdout);
		}
...
template<class VFLTR> int	DOWNSAMPLETB<VFLTR>::operator[](const int tap) {
	...
	if ((tap < 0)||(tap >= 2*NTAPS()))
		...
		int	maxinput = 3*NTAPS(), nlen;
		nlen = maxinput;
		this->m_hk  = new int64_t[nlen];
		int64_t *testk = new int64_t[nlen];

		for(int k=0; k<nlen; k++)
			this->m_hk[k] = 0;

		// Create an input vector with a single impulse in it
		for(int sub=0; sub<NDOWN(); sub++) {
...
assert(nlen <= maxinput);
			...
			for(int i=0; i<nlen; i++) {
				testk[i] >>= shift;
// assert(sub+i*NDOWN() <= maxinput);
if (sub+i*NDOWN() < maxinput)
				this->m_hk[sub+i*NDOWN()] = -testk[i];
			}
		}
	...
	for(int k=0; k<nlen; k++) {
		...
		if (data[k] != m) {
			printf("Err: Data[%*d] = %8ld != (*this)[%*d] = %8d\n",
				(int)ceil(log(nlen+1)/log(10.0)), k, data[k],
				(int)ceil(log(nlen+1)/log(10.0)), k, m);
			// mismatch = true;
		}
	}
